...
export default function PresentStatusPage() {
  ...
  const callAboutPage = async () => {
    try {
      const res = await fetch("/presentstatuspage", {
        method: "GET",
        headers: {
  ...
  const fetchBooks = async () => {
    try {
      if (userData) {
        ...
        const prevBorrowedBook = previously_borrowed_books.map((bookId) =>
          axios.get(`/books/${bookId}`).then((response) => response.data)
        );
        ...
        const prevBorrowed = await Promise.all(prevBorrowedBook);
    ...
    <>
      ...
      <div className="container py-3">
        <div className="container align-items-start my-2">
          <h5>
            PresentStatus <span style={{ color: "red" }}>Page</span>
          </h5>
        </div>
        <div
        ...
        >
          ...
          <h4 id="scrollspyHeading2">Previous Borrowed Books</h4>
          <div className="row py-2 g-5 my-2 justify-content-around">
            {prevBorrowed.map(fnBorrowedBooks)}
          </div>
